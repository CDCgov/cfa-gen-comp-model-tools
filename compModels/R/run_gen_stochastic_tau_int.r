#' Runs generalized stochastic compartmental model with tau leaping and
#' intervention
#'
...
#' @param modifier_vec value to be multiplied against parameter vector
...
#'   dplyr::expr(gamma * i)
#' )
#' out <- run_gen_stochastic_tau_int(
...
#'   parms_vec[["n"]],
#'   n_sims,
#'   intervention_start_time = 15,
#'   intervention_end_time = 20,
#'   modifier_vec = c(5)
#' )
#' plot_stoch_model(out, time_var = "time")
#' }
run_gen_stochastic_tau_int <- function(init_vals, transitions,
                                       rate_func,
  ...
  sims <- lapply(1:n_sims, function(i) {
    adaptivetau::ssa.exact(
      ...
      transitions = transitions,
      rateFunc = function(state, params, t) {
        if (!is.null(
          intervention_start_time
        ) &&
          !is.null(intervention_end_time) &&
          !is.null(modifier_vec)) {
          if (t >= intervention_start_time && t <= intervention_end_time) {
            params <- params * modifier_vec
          }
        }
        rate_func(state, params, t)
      },
      params = parms_vec,
      tf = t
    )
