test_that("scaleprocesstblbygroups scales processes right", {
  tblprocess <- tibble::tibble(
    rate = c("1", "2", "3"),
    processname = c("inter1", "inter2", "inter3"),
    processgroup = c("grp1", "grp1", "grp2"),
    processlabel = c("interaction", "interaction", "interaction"),
    Age = c("Old", "Young", "Old")
  )
  tblgroups <- tibble::tibble(
    groupname = c("Old", "Young"),
    grouptype = c("Age", "Age"),
  ...
  tblprocess_scaled <- scaleprocesstblbygroups(tblprocess, tblgroups, scalecol)
  expect_equal(
