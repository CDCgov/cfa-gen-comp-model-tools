...
# config <- yaml::read_yaml("compModels/vignettes/config-basic.yml") #nolint
...
plot_determin_model(modelout2[, -5],
  ...
  intervention_period = c(
    config$deterministic$intervention_start_time,
    config$deterministic$intervention_end_time
  )
)
```
...
# infected compartment (eg. i1, i2, etc). You can also print more than one
# compartment.
plot_stoch_model(modelout3)
plot_stoch_model(modelout3, compartments = c("s", "i"))
```
...
plot_stoch_model(modelout4, time_var = "time", compartments = c("i", "s"))
```
...
# Error differing argument lengths

# nolint start
# calculate_initial_growth_rate(modelout, time_var = "time", i_var = "i",
#                               beta = config$deterministic$beta,
#                               gamma = config$deterministic$gamma,
#                               lower_time = 10,
#                               upper_time = 25)
...
# no intervention
calculate_initial_growth_rate(modelout3,
  time_var = "t", i_var = "i",
  beta = config$stochastic$beta,
  gamma = config$stochastic$gamma,
  lower_time = 1,
  upper_time = 5
)
```
